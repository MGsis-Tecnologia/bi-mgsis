...
export const estoque: HelpSection = {
  ...
  localFilters:
    "Clique num card de status (Ruptura/Risco/Sem giro/Excesso) filtra a tabela de detalhamento por aquele status — é o mesmo filtro do dropdown \"Status\" que fica em cima da tabela, só um atalho visual pros mesmos cinco valores. O dropdown \"Cobertura\" filtra pela mesma faixa do donut (Sem cobertura, Fora de análise, Até 1 mês ... Mais de 12 meses). Busca por SKU/descrição/fabricante roda no servidor (o catálogo tem dezenas de milhares de itens, grande demais para filtrar no navegador) e se combina com os dois dropdowns.",
  metrics: [
    {
      id: "cobertura",
    ...
    },
    {
      ...
      logic:
        "Ruptura = estoque zerado mas com saída no período (era vendido e acabou). Risco = cobertura ≤ 15 dias. Excesso = cobertura ≥ 180 dias. Sem giro = estoque positivo e zero saída no período.",
    },
    {
      id: "categoria",
    ...
    },
    {
      ...
      filters:
        "Dropdown de Status, dropdown de Cobertura e busca por texto — os três se combinam (ex.: Status=Excesso + Cobertura=\"Mais de 12 meses\" + busca=\"filtro\" mostra só o que bate com tudo isso ao mesmo tempo). O botão \"Excel\" ao lado da busca baixa em .xlsx exatamente as linhas que estão passando por esses filtros no momento, com todas as colunas da tabela — útil pra levar um recorte específico (ex.: só o que está em risco) pra fora do sistema.",
      columns: [
      ...
      howToRead:
        "\"999+m\" na coluna Cobertura não é exatamente 999 meses — é o teto de exibição pra quando a demanda no período é tão baixa que a conta daria um número absurdamente alto; o item claramente está parado, o número exato não muda a conclusão.",
    },
  ],
};
